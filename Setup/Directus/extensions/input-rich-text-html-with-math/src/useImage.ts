...
// Extension-compatible utility functions
function getPublicURL(): string {
	// For extensions, we'll use a relative path or the current origin
	return window.location.origin + '/';
}

function addQueryToPath(path: string, queries: Record<string, any>): string {
...
function readableMimeType(mimeType: string, includeExtension = false): string {
	const extension = mimeType.split('/')[1];
	return includeExtension ? extension : mimeType;
}
...
export default function useImage(
	...
	function onImageSelect(image: File) {
		...
		const fileTypeExtension = image.type
			...
			: readableMimeType(image.type || 'image/jpeg', true);
